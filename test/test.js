...
test('reassigned args', function (t) {
	...
	var agent = caw(undefined, {protocol: 'http'});
...
test('setup https proxy', function (t) {
	var opts = {
		key: fs.readFileSync(path.join(__dirname, 'fixtures/privatekey.pem'), 'utf8'),
		cert: fs.readFileSync(path.join(__dirname, 'fixtures/certificate.pem'), 'utf8')
	};

	httpsProxy = https.createServer(opts, function () {});
...
test('tunnel methods', function (t) {
	...
	_caw('http://0.0.0.0:8000', {protocol: 'https'});
	_caw('https://0.0.0.0:5000', {protocol: 'http'});
	_caw('https://0.0.0.0:5000', {protocol: 'https'});
